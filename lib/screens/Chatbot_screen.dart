...
// 1. IMPORT the flutter_dotenv package
...
class _ChatBot extends State<ChatBot> with SingleTickerProviderStateMixin {
  ...
  // 2. REMOVED the hardcoded _geminiApiKey variable
  // final String _geminiApiKey = '...'; // <-- This is GONE

  late GenerativeModel _geminiModel;
  // 3. ADD a variable to track if the key was loaded
  bool _apiKeyFound = false;
  ...
  void initState() {
    ...
    // 4. LOAD the key securely in initState
    final String geminiApiKey = dotenv.env['GEMINI_API_KEY'] ?? '';

    if (geminiApiKey.isEmpty) {
      print("FATAL ERROR: GEMINI_API_KEY not found in .env file.");
      _apiKeyFound = false;
      // Add an error message to the chat list to inform the user
      _messages.add({
        'id': 'error',
        'text': 'Error: Could not connect to assistant (API Key not found). Please restart the app after setup.',
        'sender': 'bot',
      });
    } else {
      _apiKeyFound = true;
      _geminiModel = GenerativeModel(
        ...
        apiKey: geminiApiKey, // Use the key loaded from .env
      );
    }
    // --- End of key loading ---
  ...
  void _sendMessage() async {
    // 5. BLOCK sending messages if the key was never found
    if (!_apiKeyFound) {
      setState(() {
        _messages.add({
          'id': 'error_send',
          'text': 'Error: Cannot send message. API Key not configured.',
          'sender': 'bot',
        });
      });
      _scrollToBottom();
                    ...
                    Stack(
                      children: [
                          ...
                          child: Row(
                            children: [
                              CircleAvatar(
                                ...
                                child: Icon(
                                  Icons.psychology_alt, // AI/brain-related icon
                                  color: Color(0xFF4CAF50),
                                  size: 30,
                                ),
                              ),
                        ...
                        Positioned(
                          ...
                          child: IconButton(
                            icon: const Icon(
                              Icons.cancel_rounded, // Modern close icon
                              color: Colors.white,
                              size: 24,
                            ),
                            onPressed:
