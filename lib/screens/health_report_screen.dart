...
class HealthReportScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String severity = "Low";
    if (report.probability > 0.75) severity = "High";
    else if (report.probability > 0.4) severity = "Medium";
  ...
  Widget _buildDiseaseStatusCard(AppLocalizations localizations, String severity, bool isHealthy) {
    final color = isHealthy ? AppTheme.healthyColor : AppTheme.affectedColor;
    final title = isHealthy ? 'Plant is Healthy' : localizations.diseaseDetected;
        ...
        child: Row(
          ...
          children: [
            ClipRRect(
              ...
              child: Image.file(File(report.imagePath), width: 100, height: 100, fit: BoxFit.cover),
            ),
            const SizedBox(width: 16),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _DetailText(label: localizations.diagnosis, value: report.diseaseName),
                  const SizedBox(height: 8),
                  _DetailText(label: "Confidence:", value: "${(report.probability * 100).toStringAsFixed(1)}%"),
                  const SizedBox(height: 8),
                  _DetailText(label: localizations.recommendedAction, value: "Further investigation needed."),
                ],
              ),
            ),
          ],
